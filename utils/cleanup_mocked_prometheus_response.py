...
if response.status_code == 200:
    ...
    with open("build/docker/wiremockMappings/redirectPrometheus.json", "w", newline='\n') as f:
        # print multiline string
        content = f"""{{
  "request": {{
    "method": "GET",
    "url": "/federate?{urlPrometheusQueries}"
  }},
  "response": {{
    "status": 200,
    "headers": {{
    ...
    "bodyFileName": "redirectPrometheusResponse.txt"
  }}
}}
"""
        f.write(content)

# Kube state metrics response
url = f'https://{prometheushost}/federate?{urlKubeStateMetricsQueries}'
response = requests.get(url)
if response.status_code == 200:
    print("Successfully downloaded!")
    lines = [line.decode() for line in response.content.splitlines()]
    ...
    with open("build/docker/wiremockFiles/redirectKubeStateMetricsResponse.txt", "w", newline='\n') as f:
        for line in replaced_result:
            f.write(line + '\n')
    
    with open("build/docker/wiremockMappings/redirectKubeStateMetrics.json", "w", newline='\n') as f:
        # print multiline string
        content = f"""{{
  "request": {{
    "method": "GET",
  ...
  "response": {{
    ...
    "bodyFileName": "redirectKubeStateMetricsResponse.txt"
  }}
}}
"""
        f.write(content)
