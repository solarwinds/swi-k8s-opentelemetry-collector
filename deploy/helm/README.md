...
- [Features](#features)
  ...
  - [Auto Instrumentation (disabled by default)](#auto-instrumentation-disabled-by-default)
...
MetricsCollector --> X[" "]:::invisible --> |"ingest"| SWO_Interface
...
### Metrics Collection
Enabled by default, to disable: `otel.metrics.enabled: false`

Collects metrics from core Kubernetes resources and workloads. Collects metrics following prometheus endpoints:
* kube-state-metrics - see metrics with `k8s.kube_` prefix in [exported_metrics](docs/exported_metrics.md)
* cAdvisor - see metrics with `k8s.container_` prefix in [exported_metrics](docs/exported_metrics.md)
* apiserver - see metrics with `k8s.apiserver_` prefix in [exported_metrics](docs/exported_metrics.md)
...
* certmanager
* composite metrics - Those are create by SWO K8s Collector using metrics above. See metrics in [exported_metrics](docs/exported_metrics.md) with following prefixes: 
    * `k8s.cluster.`
    * `k8s.node.`
    * `k8s.deployment.`
    * `k8s.kube.job`
...
Additionally there are following metrics being collected:

#### Application metrics from Prometheus endpoints
##### Annotation based discovery
Enabled by default, to disable: `otel.metrics.autodiscovery.prometheusEndpoints.enabled: false`

SWO K8s Collector perform discovery of application prometheus endpoint, based on following annotations on the workloads:
...
##### (Optional) Discovery based on Pod/Service monitors
...
### Logs Collection
Enabled by default, to disable: `otel.logs.enabled: false`

It collects these logs:

#### Container logs
to disable: `otel.logs.container: false`
...
#### Journal logs
Ro disable: `otel.logs.journal: false`
...
### Events Collection
Enabled by default, to disable: `otel.events.enabled: false`

Collects all kubernetes events from the cluster and emits them as OTEL logs to SWO. Each event has following resource attributes:
* `k8s.cluster.name` - name of the cluster (input generated during onboarding)
* `sw.k8s.agent.manifest.version` - version of the manifest
...
Enabled by default, to disable: `otel.manifests.enabled: false`
...
By default, `swo-k8s-collector` collects all manifests from types above. You can use the `otel.manifests.filter` setting to filter out manifests that should not be collected.
...
SWO K8s Collector must be instructed that it runs on AWS Fargate with `aws_fargate.enabled: true`. 
It makes sure that no operation is dependent on DaemonSet (metrics and logs).

To collect logs from Fargate it needs to enable: `aws_fargate.logs.enabled: true` and follow [this documentation](https://documentation.solarwinds.com/en/success_center/observability/content/configure/configure-logs-k8s-fargate.htm)

### eBPF Network Observability
Collect metrics of network communication between workloads. Enabled by default, to enable: `ebpfNetworkMonitoring.enabled: false`

### OpenShift Support (disabled by default)
Adds support for OpenShift-specific configurations.

To enable: `openshift.enabled: true`

### Auto Update (disabled by default)
Automatically updates the Helm chart to the latest version.

To enable: `autoupdate.enabled: true`
